...
def getActorPhoto(htmlcode):  # //*[@id="star_qdt"]/li/a/img
    ...
    for i in a:
        ...
        p = str(html.xpath(
            '//*[@id="waterfall"]/div[1]/div/div[1]/img/@src')).strip(" ['']")
        p2 = {t: p}
        d.update(p2)
    return d
...
def getStudio(htmlcode):  # 获取厂商
    ...
    result = str(html.xpath(
        '/html/body/div[5]/div[1]/div[2]/p[5]/a/text()')).strip(" ['']")
    return result
...
def getNum(htmlcode):  # 获取番号
    html = etree.fromstring(htmlcode, etree.HTMLParser())
    result = str(html.xpath(
        '/html/body/div[5]/div[1]/div[2]/p[1]/span[2]/text()')).strip(" ['']")
    return result
...
def getDirector(htmlcode):  # 获取导演
    html = etree.fromstring(htmlcode, etree.HTMLParser())
    result = str(html.xpath(
        '/html/body/div[5]/div[1]/div[2]/p[4]/a/text()')).strip(" ['']")
    return result
...
def getSerise(htmlcode):
    html = etree.fromstring(htmlcode, etree.HTMLParser())
    result = str(html.xpath(
        '/html/body/div[5]/div[1]/div[2]/p[7]/a/text()')).strip(" ['']")
    return result
...
def main(number):
    try:
        htmlcode = get_html(JAVBUS_ADDREDSS + number)
        try:
            dww_htmlcode = get_html(
                "https://www.dmm.co.jp/mono/dvd/-/detail/=/cid=" + number.replace("-", ''))
        except:
            dww_htmlcode = ''
        dic = {
            ...
            'website': JAVBUS_ADDREDSS + number,
            'source': 'javbus.py',
        }
...
def main_uncensored(number):
    ...
    if getTitle(htmlcode) == '':
        htmlcode = get_html(JAVBUS_ADDREDSS +
                            number.replace('-', '_'))
        dww_htmlcode = get_html(
            "https://www.dmm.co.jp/mono/dvd/-/detail/=/cid=" + number.replace("-", ''))
    dic = {
